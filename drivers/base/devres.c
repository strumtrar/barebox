...
static inline void *kmalloc_node_track_caller(size_t size, gfp_t gfp)
{
					       ...
					       size_t size, gfp_t gfp)
{
	...
	dr = kmalloc_node_track_caller(tot_size, gfp);
	if (unlikely(!dr))
		return NULL;
 ...
 */
static void *__devres_alloc_node(dr_release_t release, size_t size, gfp_t gfp,
			  const char *name)
{
	struct devres *dr;
 ...
 */
static void devres_free(void *res)
{
	if (res) {
 ...
 */
static void devres_add(struct device *dev, void *res)
{
	struct devres *dr = container_of(res, struct devres, data);
	unsigned long flags;
 ...
 */
static int devres_destroy(struct device *dev, dr_release_t release,
			  dr_match_t match, void *match_data)
{
	void *res;
 ...
 * 0 if devres is found and freed, -ENOENT if not found.
 */
static int devres_release(struct device *dev, dr_release_t release,
			  dr_match_t match, void *match_data)
{
	void *res;
...
int devres_release_all(struct device *dev, bool shutdown)
{
	...
	/* We currently only allocate memory, so omitting that on shutdown is
	 * completely fine
	 */
	if (shutdown)
		return 0;
 ...
 * resources, guaranteed alignment is ARCH_DMA_MINALIGN.
 *
 * RETURNS:
...
/**
 * devm_xmalloc - Resource-managed xmalloc
 * @dev: Device to allocate memory for
 * @size: Allocation size
 * @gfp: Allocation gfp flags
 *
 * Managed xmalloc.  Memory allocated with this function is
 * automatically freed on driver detach.  Like all other devres
 * resources, guaranteed alignment is ARCH_DMA_MINALIGN.
 *
 * RETURNS:
 * Pointer to allocated memory on success, Panic on failure
 */
void *devm_xmalloc(struct device *dev, size_t size, gfp_t gfp)
{
	void *ptr;
